...
TOR_PASSWORD = "tor_password"            # The Tor password is written in the docker-compose.yml file. (It is passed as a build argument to the 'tor' service).
...
HTTP_PROXY = 'http://localhost:8118'.format(address=PRIVOXY_ADDRESS, port=PRIVOXY_PORT)
    ...
    def change_ip(self):
        ...
        while True:
            ...
            if new_ip == current_ip:
                logger.debug("The IP address: (" + current_ip + ") is still the same. Waiting for 1 second before checking again...")
                time.sleep(1)
            else:
                break
